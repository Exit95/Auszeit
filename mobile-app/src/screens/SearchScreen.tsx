...
export function SearchScreen() {
  ...
  const handleSearch = useCallback(async () => {
    if (!query.trim() && !filterStatus) return;
    ...
    try {
      ...
      const ordersResult = await api.get<any>(`/orders?${params.toString()}`);
      const ordersList = ordersResult?.data?.orders || ordersResult?.data || ordersResult || [];
      (Array.isArray(ordersList) ? ordersList : []).forEach((o: Order) => combined.push({ type: 'order', data: o }));

      // Kunden suchen (nur bei Textsuche)
      if (query.trim() && !filterStatus) {
        const custResult = await api.get<any>(`/customers?search=${encodeURIComponent(query.trim())}`);
        const custList = custResult?.data?.customers || custResult?.data || custResult || [];
        (Array.isArray(custList) ? custList : []).forEach((c: Customer) => combined.push({ type: 'customer', data: c }));
      }
