...
const STATUS_FILTERS: (OrderStatus | 'alle')[] = [
  'alle', 'neu', 'geplant', 'im_ofen', 'gebrannt', 'abholbereit', 'abgeschlossen', 'storniert',
];
...
export function OrdersScreen() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      const params = activeFilter !== 'alle' ? `?status=${activeFilter}` : '';
      const result = await api.get<Order[]>(`/orders${params}`);
      setOrders(result);
    } catch (error) {
